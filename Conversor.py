...
formato_img = str(input("Formato da imagem (jpg, png, ...): "))
...
escala = int(input("Defina a escala que a imagem possuíra (0.5, 0.6, ..., 1): "))

# Opencv(cv2) com método de interpolação por área
array_redi = cv2.resize(array, fx = escala, fy = escala,  interpolation = cv2.INTER_AREA)

# Deixando preto e branco (Fórmula ITU-R BT.709)
array_redi = 0.2125*array_redi[:,:,0] + 0.7153*array_redi[:,:,1] + 0.0721*array_redi[:,:,2]

plt.imshow(array_redi)
plt.axis("off")
plt.show()
